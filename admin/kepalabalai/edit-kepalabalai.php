<?php 
	...
	$sql = mysqli_query($con, "SELECT * FROM tbl_kepalabalai WHERE id_kepalabalai='$id'");
	...
	if(isset($_POST['submit'])) {
		...
		$data = mysqli_fetch_array($sql);
			...
			if(!empty($gambar)) {
				...
				$gambar = $data['img'];
				$sql = mysqli_query($con, "UPDATE tbl_posts SET img='$gambar', judul='$judul', artikel='$artikel', kategori='$kategori' WHERE id_kepalabalai='$id'");
				echo "<script>alert('Data Berhasil Di ubah!')</script>";
				echo "<script>window.location.href='index.php?page=tampil-berita'</script>";
			}
	}
