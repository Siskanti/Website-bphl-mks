			...
			<div class="card-body">
				<table class="table table-bordered">
					...
					?>
						<tr>
							<th><?= $no++; ?></th>
							<th class="text-center"><img src="../assets/imgg/<?= $data['nama'] ?>" width="180" height="100"></th>
							<th class="text-center">
								<a href="index.php?page=hapus-kontak&id=<?= $data['id_imgg'] ?>" class="btn btn-danger">
...
if (isset($_POST['submit'])) {
	...
	if (!empty($gambar)) {
		if (in_array($ekstensi, $ekstensi_boleh) === true) {
			if ($ukuran < 5000000) {
				move_uploaded_file($file_tmp, '../assets/imgg/' . $gambar);
				$sql = mysqli_query($con, "INSERT INTO tbl_kontakk VALUES ('', '$gambar')");
